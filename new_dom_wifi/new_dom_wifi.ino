...
void setup() {
  ...
  butt1.setClickTimeout(30);   // настройка таймаута между кликами (по умолчанию 300 мс)
}
...
void loop() {
  ...
  if (flag_open && (millis() > (fot + 10000))) {
    Serial.println("n0");  //Таймер(10с) чтоб дверь сама закрывалась
    flag_open = false;
  }

  if (Firebase.getString("consol") == "1") {
    flagg = true; tone(piezoPin, 500, 500); //Если был запрос дистанционного открытия
    Serial.print("d1"); Firebase.setString("sost", "Открыто дистанционно");
  }//Уведомить обратно и открыть сервер
  else if (flagg) {
    Serial.print("d0");
    flagg = false;
    Firebase.setString("sost", "Закрыто");
  }
  if (Firebase.getString("Svtilo_kon") != Svetil_g) {
    Svetil_g = Firebase.getString("Svtilo_kon");
    for (int i = 0; i < 2; i++) {
      digitalWrite(pin_key, 0);
      delay(20);
      digitalWrite(pin_key, 1);
      delay(20);
    }
  }
  if (Firebase.getString("Door_kon") != Door_g) {
    Door_g = Firebase.getString("Door_kon");
    digitalWrite(pin_key, 0); delay(20); digitalWrite(pin_key, 1); delay(20);
  }
...
void chek() {
  if (butt1.hasClicks()) {
    switch (butt1.getClicks()) {
      case 1:
        Firebase.setString("svetilo", "Работает");
        break;
      case 2:
        Firebase.setString("svetilo", "Отключен");
        break;
      case 3:
        Firebase.setString("vorota", "Открыты");
        break;
      case 4:
        Firebase.setString("vorota", "Закрыты");
        break;
    }
  }
}
